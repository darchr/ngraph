...
void runtime::gpu::GPUExternalFunction::emit_functions()
{
    for (const auto& p : m_function_ordered_ops)
    {
        ...
        m_writer.block_begin();
        {
            ...
            // TIMER: emit CPU Timer for debugging
            //m_writer << "Timestamp start_ts;\n";
            ...
            for (shared_ptr<Node> node : m_function_ordered_ops.at(current_function))
            {
                ...
                // TIMER: Synchronize and stop timer.
                //m_writer << "runtime::gpu::wait_barrier(ctx, false);\n"; 
                //m_writer << "std::cout << \"Time: " << node->get_name() << " -- \" << "
                //         << "(std::chrono::duration_cast<Timescale>(Clock::now() - start_ts)).count() << std::endl;\n";
